...
- **Яндекс Метрика** — inline в nuxt.config.ts, metrika ID переопределяем через ENV; цели через `useMetrikaGoal().reachGoal()` (brief_submit, booking_click, sticky_cta_click, bankimport_click, priceimport_click, card_copy_link, card_qr_reveal)
